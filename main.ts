...
function setupMarkerIconAutocomplete() {
  const input = document.getElementById('marker-icon') as HTMLInputElement | null;
  ...
  input.oninput = function() {
    const val = (input as HTMLInputElement).value.trim().toLowerCase();
    list.innerHTML = '';
    if (!val) return;
    ...
    matches.forEach(name => {
      ...
      item.onclick = function() {
        ...
        (preview as HTMLElement).textContent = name;
        list.innerHTML = '';
...
function addDoubleClickToLayer(layer: L.Layer) {
  const type = getObjectType(layer);
  // --- тултіп ---
  ...
  function showTooltip(e: any) { // @ts-ignore
    ...
    if (e.originalEvent && typeof e.originalEvent.clientX === 'number' && typeof e.originalEvent.clientY === 'number') {
      ...
      const point = (map as any).latLngToLayerPoint(e.latlng); // @ts-ignore
